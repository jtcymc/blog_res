!function(a, b) {
    ...
    function i(a) {
        ...
        g = g && parseInt(g >= 0) && parseInt(g) <= Math.NEGATIVE_INFINITY ? parseInt(g) : e.timeout,
        b.timeout = g,
        ...
        i.innerHTML = '<div class="qmsg-content">            <div class="' + l + '">                <i class="qmsg-icon">' + k + "</i>" + q.outerHTML + p + "</div>		</div>",
        ...
        b.$elem = i,
        j(b, "opening"),
        b.settings.showClose && i.querySelector(".qmsg-icon-close").addEventListener("click", function() {
            b.close()
        }
        .bind(i)),
        i.addEventListener("animationend", function(a) {
            var b = a.target
              , c = a.animationName;
            c === d["closing"] && (clearInterval(this.timer),
            this.destroy()),
            b.style.animationName = ""
        }
        .bind(b)),
        b.settings.autoClose && (s = 10,
        ...
        .bind(b), s),
        b.$elem.addEventListener("mouseover", function() {
            clearInterval(this.timer)
        }
        .bind(b)),
        b.$elem.addEventListener("mouseout", function() {
    ...
    function j(a, b) {
        b && d[b] && (a.state = b,
        a.$elem.style.animationName = d[b])
    }
    function k(a) {
        var b = h("count")
    ...
    function m(a) {
        ...
        if (0 > c) {
            ...
            d.count = d.count ? d.count >= 99 ? d.count : d.count + 1 : 2,
            k(d);
        return d.$elem.setAttribute("data-count", d.count),
        d
    }
    var b, c, d, e, f, g, n;
    ...
    i.prototype.close = function() {
        j(this, "closing"),
        g ? n.remove(this.id) : this.destroy();
    ...
    n = {
        ...
        closeAll: function() {
            for (var a in this.oMsgs)
                this.oMsgs[a] && this.oMsgs[a].inst.close()
        }
    }
});
